...
with open('../../data/norman19_loss_weights_dict.pkl', 'wb') as f:
...
all_first_half_perts = [val.tolist() for k,val in first_half_split_dict.items()]
all_second_half_perts = [val.tolist() for k,val in second_half_split_dict.items()]
# Flatten the lists and remove duplicates
all_first_half_perts = list(set([item for sublist in all_first_half_perts for item in sublist]))
all_second_half_perts = list(set([item for sublist in all_second_half_perts for item in sublist]))
# Concatenate with 'ctrl'
all_first_half_perturbations = np.concatenate([['ctrl'], all_first_half_perts])
all_second_half_perturbations = np.concatenate([['ctrl'], all_second_half_perts])

# Create subsetted datasets
print("Creating subsetted datasets...")
adata_first_half_gears = adata_gears[adata_gears.obs['condition'].isin(all_first_half_perturbations)].copy()
adata_second_half_gears = adata_gears[adata_gears.obs['condition'].isin(all_second_half_perturbations)].copy()
...
# try:
#     pert_data_first_half.load(data_path='../../data/norman19_gears')
# except Exception as e:
#     pert_data_first_half.new_data_process(dataset_name='norman19_gears', adata=adata_first_half_gears)
#     pert_data_first_half.load(data_path='../../data/norman19_gears')

# # Do the same for the second half
# print("Creating second half subsetted dataset...")
# pert_data_second_half = PertData('../../data')
# pert_data_second_half.load(data_path='../../data/norman19_gears')
...
def train_single_model(config):
    ...
    if os.path.exists(f'../../data/gears_predictions_{loss}_{weight}_norman19.pkl'):
    ...
    with open('../../data/norman19_gears_first_half_split_dict.pkl', 'rb') as f:
        first_half_split_dict = pickle.load(f)
    with open('../../data/norman19_gears_second_half_split_dict.pkl', 'rb') as f:
    ...
    if weight == 'weighted':
        ...
        with open('../../data/norman19_loss_weights_dict.pkl', 'rb') as f:
    ...
    if model_half == 'first_half':
        ...
        gears_model.save_model(f'../../data/gears_models/norman19_first_half_{loss}_{weight}')
        ...
        with open(f'../../data/gears_predictions_{loss}_{weight}_first_half_temp.pkl', 'wb') as f:
        ...
        os.makedirs("../../data/gears_models", exist_ok=True)
        gears_model.save_model(f'../../data/gears_models/norman19_second_half_{loss}_{weight}')
        ...
        predictions = gears_model.predict(to_predict)
        with open(f'../../data/gears_predictions_{loss}_{weight}_second_half_temp.pkl', 'wb') as f:
            pickle.dump(predictions, f)
...
def combine_predictions(loss, weight):
    ...
    first_half_file = f'../../data/gears_predictions_{loss}_{weight}_first_half_temp.pkl'
    second_half_file = f'../../data/gears_predictions_{loss}_{weight}_second_half_temp.pkl'
