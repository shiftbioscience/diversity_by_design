...
try:
    ...
    def train(
        ...
        Returns:
            Tuple[float, float]: Total loss and total MSE for the epoch
        """
        model.train()
    ...
    def eval_perturb(
        ...
        results["pred"] = pred.detach().cpu().numpy().astype('float32')
        results["truth"] = truth.detach().cpu().numpy().astype('float32')

        pred_de = torch.stack(pred_de)
        truth_de = torch.stack(truth_de)
        results["pred_de"] = pred_de.detach().cpu().numpy().astype('float32')
        results["truth_de"] = truth_de.detach().cpu().numpy().astype('float32')
    ...
    def predict(
        ...
        Args:
            ...
            pert_data: The perturbation data object containing adata and gene names
    ...
    print("Warning: scGPT dependencies were not able to be loaded")
# ===================SCGPT DEPENDENCIES LOAD ===================
# ===================SCGPT DEPENDENCIES LOAD ===================
# ===================SCGPT DEPENDENCIES LOAD ===================